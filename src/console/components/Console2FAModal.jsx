...
export default function Console2FAModal({ onClose }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // A failed read must not look like "2FA is not enabled".
      setLoadFailed(true)
      setError(toUserMessage(err, 'Could not read your 2FA status.'))
    } finally {
      setLoading(false)
    }
  }, [listMfaFactors])
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}
        className="w-full max-w-md bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] focus:outline-none">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p id={titleId} className="text-sm font-bold text-white">Two-Factor Authentication</p>
              <p className="text-[10px] text-gray-500">TOTP via Google Authenticator or Authy</p>
            </div>
          </div>
          <button type="button" onClick={onClose} aria-label="Close" title="Close" className="rounded text-gray-500 hover:text-gray-300 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {loading && step === 'status' ? (
            <div className="flex items-center justify-center h-24" role="status" aria-label="Loading 2FA status">
              <div className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" aria-hidden="true" />
            </div>
          ) : (
            <>
              {error && (
                <div role="alert" className="flex items-center gap-2 p-3 rounded-lg bg-red-950/50 border border-red-800/50 mb-4">
                  <AlertTriangle size={13} className="text-red-400 flex-shrink-0" aria-hidden="true" />
                  <p className="text-xs text-red-300 flex-1 min-w-0 break-words">{error}</p>
                  {loadFailed && (
                    <button type="button" onClick={() => { setError(null); load() }}
                      className={`text-[11px] px-2 py-0.5 rounded border border-red-700/50 text-red-200 hover:bg-red-900/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500`}>
                      Retry
                    </button>
                  )}
                </div>
              ...
              {success && (
                <div role="status" className="flex items-center gap-2 p-3 rounded-lg bg-green-950/50 border border-green-800/50 mb-4">
                      ...
                      <div className="space-y-2">
                        {verifiedFactors.map(f => (
                          <div key={f.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-800/60 border border-gray-700">
                            ...
                            <button type="button" onClick={() => { setRemoveId(f.id); setStep('unenroll_confirm'); setError(null); setSuccess(null) }}
                              aria-label={`Remove ${f.friendly_name || 'Authenticator App'}`}
                              className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 text-[10px] px-2.5 py-1 rounded-lg bg-red-900/30 text-red-400 hover:bg-red-900/50 border border-red-700/40 transition-colors">
                              Remove
                            </button>
                          </div>
                        ))}
              ...
              {step === 'enroll_qr' && enrollData && (
                <div className="space-y-4">
                  ...
                  <div className="flex justify-center p-4 bg-white rounded-2xl">
                    <img src={enrollData.qrCode} alt="QR code to scan with your authenticator app" className="w-48 h-48" />
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1.5">Manual entry key (if camera unavailable)</p>
                    <div className="flex items-center gap-2 p-3 rounded-lg bg-gray-800 border border-gray-700">
                      ...
                      <button type="button" onClick={() => setShowSecret(s => !s)}
                        aria-label={showSecret ? 'Hide key' : 'Show key'} title={showSecret ? 'Hide key' : 'Show key'}
                        className={`rounded text-gray-500 hover:text-gray-300 flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500`}>
                        {showSecret ? <EyeOff size={13} /> : <Eye size={13} />}
                      </button>
                      <button type="button" onClick={copySecret}
                        aria-label={copied ? 'Key copied' : 'Copy key'} title={copied ? 'Copied' : 'Copy key'}
                        className={`rounded text-gray-500 hover:text-gray-300 flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500`}>
              ...
              {step === 'enroll_verify' && (
                <div className="space-y-4">
                  ...
                  <div>
                    <p id={`${titleId}-code`} className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-3 text-center">
                      Verification Code
                    </p>
                    <div className="flex justify-center gap-1.5 sm:gap-2" role="group" aria-labelledby={`${titleId}-code`}>
                      {code.map((digit, i) => (
                        <input key={i} ref={el => inputRefs.current[i] = el}
                          type="text" inputMode="numeric" maxLength={6} autoComplete={i === 0 ? 'one-time-code' : 'off'}
                          aria-label={`Digit ${i + 1} of 6`}
                          value={digit}
                          onChange={e => handleCodeInput(i, e.target.value)}
                          onKeyDown={e => handleCodeKey(i, e)}
                          onPaste={e => { e.preventDefault(); handleCodeInput(i, e.clipboardData.getData('text')) }}
                          className={`w-9 sm:w-10 h-12 text-center text-lg font-bold rounded-xl border transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${
        ...
        <div className="flex gap-2 px-6 py-4 border-t border-gray-800">
          {step === 'status' && !has2FA && !loadFailed && (
            <>
              <button type="button" onClick={onClose}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs text-gray-400 bg-gray-800 hover:bg-gray-700 transition-colors">
                Close
              </button>
              <button type="button" onClick={startEnroll} disabled={loading}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs font-semibold text-white disabled:opacity-40 transition-colors"
          ...
          {step === 'enroll_qr' && (
            <>
              <button type="button" onClick={() => { setStep('status'); setEnrollData(null); setError(null) }}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs text-gray-400 bg-gray-800 hover:bg-gray-700 transition-colors">
                Cancel
              </button>
              <button type="button" onClick={() => { setStep('enroll_verify'); setError(null); setCode(['','','','','','']) }}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs font-semibold text-white"
                style={{ background: 'linear-gradient(135deg,#1d4ed8,#3b82f6)' }}>
          ...
          {step === 'enroll_verify' && (
            <>
              <button type="button" onClick={() => { setStep('enroll_qr'); setError(null) }}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs text-gray-400 bg-gray-800 hover:bg-gray-700 transition-colors">
                Back
              </button>
              <button type="button" onClick={handleVerify} disabled={loading || code.join('').length !== 6}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs font-semibold text-white disabled:opacity-40"
                style={{ background: 'linear-gradient(135deg,#1d4ed8,#3b82f6)' }}>
          ...
          {step === 'unenroll_confirm' && (
            <>
              <button type="button" onClick={() => { setStep('status'); setError(null) }}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs text-gray-400 bg-gray-800 hover:bg-gray-700 transition-colors">
                Cancel
              </button>
              <button type="button" onClick={() => handleUnenroll(removeId || verifiedFactors[0]?.id)} disabled={loading}
                className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 flex-1 py-2 rounded-lg text-xs font-semibold text-white bg-red-700 hover:bg-red-600 disabled:opacity-40 transition-colors">
