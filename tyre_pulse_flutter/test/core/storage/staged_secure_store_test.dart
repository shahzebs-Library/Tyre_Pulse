...
void main() {
  ...
  StagedSecureStore store({
    int chunkSize = StagedSecureStore.defaultChunkSize,
  }) {
    return StagedSecureStore(
      slots: fake,
      chunkSize: chunkSize,
      sleep: _noSleep,
    );
  ...
  group('chunking', () {
    test('splits a long value without cutting a surrogate pair, and '
      ...
      final chunkEntries =
          fake.raw.entries.where((e) => e.key.contains('_chunk_')).toList()
            ..sort((a, b) {
      ...
      for (final entry in chunkEntries) {
        expect(
          _startsWithLowSurrogate(entry.value),
          isFalse,
          reason: 'chunk "${entry.key}" starts mid-surrogate-pair',
        );
        expect(
          _endsWithHighSurrogate(entry.value),
          isFalse,
    ...
    test('a value at or under the chunk size is not chunked at all', () async {
  ...
  group('the commit point', () {
    test(
      'an interrupted write leaves the PREVIOUS value fully readable',
      () async {
        final sut = store(chunkSize: 4);
        ...
        final chunkWritesForSecondAttempt = fake.calls
            .where(
              (c) => c.startsWith('write:session_g') && c.contains('_chunk_'),
            )
            .length;
  ...
  group('delete', () {
    test('removes metadata, then the plain slot, then the chunks, in that '
      ...
      await sut.write(
        'queue',
        'this value is long enough to be split into '
            'several chunks',
      );
      fake.calls.clear();
    ...
    test(
      'a plain-slot delete failure is reported, not swallowed, because a '
  ...
  group('read retry', () {
    test('a slot that refuses every read is retried up to '
      ...
      final attempts = fake.calls.where((c) => c == 'read:session_meta').length;
  ...
  group('unreadable and torn are both failures, and neither is absent', () {
    ...
    test(
      'torn, produced by a genuinely committed value missing a chunk',
      () async {
        final sut = store(chunkSize: 4);
  ...
  group('absent vs failed', () {
    test(
      'a key that was never written reports absent, not a failure',
      () async {
        final sut = store();
  ...
  group('key validation', () {
    test(
      'an empty key is a programming error, not a runtime condition',
      () async {
        final sut = store();
