/**
 * LegacyLayout.jsx - THE PRE-REWRITE APP SHELL, KEPT ONLY AS A ROLLBACK PATH.
 *
 * This is the shell as it stood at commit 1e2584f7, before the desktop TopBar
 * took over the global controls (search, working context, language,
 * notifications, theme, profile, version, sign-out) and before the sidebar was
 * regrouped from 10 flat groups into 13 groups with sub-headings.
 *
 * It is reachable ONLY when the `new_shell` system_config flag is explicitly
 * turned off, which flips the picker in src/App.jsx over to this component. It
 * exists so an administrator can fall back to a known-good shell in one toggle
 * instead of waiting for a revert and a deploy.
 *
 * IT IS INTENTIONALLY FROZEN. Do NOT extend it, do not tidy its dead code, do
 * not fix its lint warnings, do not add a nav item here. Every shell change
 * goes into Layout.jsx. The value of a fallback is that it is unchanged from
 * the last shell known to work; each edit here erodes exactly that.
 *
 * It deliberately exports NO NAV_CATALOG. That descriptor has ONE source,
 * Layout.jsx, because four console surfaces (CustomRolesManager,
 * ConsoleModuleControl, ConsoleNavigation, ConsolePlatformMap) and five tests
 * read it; a second exported copy here would silently drift from the sidebar
 * those screens claim to describe.
 */
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/asset-management',    label: 'Asset Management',   icon: LayoutGrid },
...
// NAV_CATALOG (the icon-free descriptor of the built-in nav that the Navigation
// Customizer, Module Control, Custom Roles and Platform Map console pages read)
// is DELIBERATELY NOT declared here. It is exported from Layout.jsx alone, so
// there is exactly one catalog and it always describes the sidebar that is
// actually shipping. Two copies would diverge the moment either shell changed,
// and the console pages would then describe a nav nobody sees.
