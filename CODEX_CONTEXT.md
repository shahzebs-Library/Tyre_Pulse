...
- Enterprise checklist evidence foundation (2026-09-21, branch `feature/checklist-enterprise-governance`): immutable published template revisions, exact snapshots on new submissions, explicit legacy/missing evidence labels and assignment-based 30-day compliance monitoring are implemented. Web detail/PDF and primary Flutter approval review prefer stored snapshots. Web register filters expose evidence gaps; schedule skips require immutable reason/actor/time; Checklist Insights drills into template/site compliance and measures supervisor/area-manager queue age from recorded workflow events. Plan and measured production baseline: `audit/enterprise-checklist-governance-plan.md`. Production schedules, approval policies, SLA thresholds, severity mappings and historical backfills remain intentionally unconfigured pending operational inputs.
