...
void main() {
  group('the underlying session phase passes straight through', () {
    test('restoring', () {
      expect(
        deriveSession(const AuthState.restoring()).phase,
    ...
    test('timedOut', () {
      expect(
        deriveSession(const AuthState.timedOut()).phase,
    ...
    test('signedOut', () {
      expect(
        deriveSession(const AuthState.signedOut()).phase,
  ...
  group('a profile that failed to load fails CLOSED', () {
    ...
    test('loaded with no profile value (a controller bug, not a reachable '
      ...
      expect(deriveSession(state).gate, TpShellGate.profileUnavailable);
    });
  });
  ...
  group('locked or unapproved reports accessBlocked', () {
    ...
    test(
      'an approved, unlocked profile is not blocked on this account alone',
      () {
        final AuthState state = AuthState(
          sessionPhase: AuthSessionPhase.authenticated,
          userId: 'user-1',
  ...
  group('the version gate only blocks on the ONE blocking reason', () {
    ...
    test(
      'buildBelowMinimum is the ONLY reason that reaches updateRequired',
      () {
        final AuthState state = AuthState(
          sessionPhase: AuthSessionPhase.authenticated,
          userId: 'user-1',
    ...
    test(
      'a version check still in flight (notChecked) never blocks sign-in',
      () {
