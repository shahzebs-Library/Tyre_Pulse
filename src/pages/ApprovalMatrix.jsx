...
export default function ApprovalMatrix() {
  ...
  const [test, setTest] = useState({ entity_type: 'inspection', country: scope, region: '', site: '', role: '', user_id: null })
  ...
  useEffect(() => {
    setPolicies([]); setPeople([]); setSites([]); setRoles([]); setForm(null); setReview(null); setHistory(null); setMessage(''); setIncludeDraft(false); setDirty(false); setTest({ entity_type: 'inspection', country: scope, region: '', site: '', role: '', user_id: null })
    load()
  ...
  const scopedPolicies = useMemo(() => policies.filter(p => (!scope || !p.match_country || p.match_country === scope) && (!stateFilter || p.state === stateFilter) && [p.name, p.entity_type, p.match_region, p.match_site, p.match_country].join(' ').toLowerCase().includes(search.toLowerCase())), [policies, scope, stateFilter, search])
  ...
  const regionOf = s => String(s?.region || '').trim().toUpperCase()
  const regionOptions = country => [...new Set(sites.filter(s => s.active !== false && (!country || s.country === country)).map(regionOf).filter(Boolean))].sort()
  const siteOptions = (country, region = '') => sites.filter(s => country && s.country === country && s.active !== false && (!region || regionOf(s) === region))
  const siteRegion = (country, name) => regionOf(sites.find(s => s.country === country && s.name === name))
