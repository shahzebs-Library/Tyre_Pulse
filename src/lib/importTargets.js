 ...
 * zero clicking.
 *
 * KNOWN LIMIT, stated honestly: these triggers do a bare INSERT. Re-importing the
 * same file inserts the rows again. Until an import-fingerprint guard lands, check
 * Console -> Duplicate Control after any re-run. That page finds, prices and undoes
...
export const IMPORT_TARGETS = Object.freeze([
  {
    ...
    notes: 'THE authoritative cost source. Pick the table for the country - the country '
      ...
      + 'Big files stall the dashboard importer because of the embedded inch marks in '
      + 'item descriptions - use the in-app Data Intake page for those.',
  },
  {
  ...
  },
  {
    ...
    columns: ['country', 'location', 'job_card_type', 'job_card_no', 'jc_status',
      ...
      'complaint'],
    notes: 'REPLACES the whole snapshot for that country on each import - it is a live '
      + 'picture of what is open, not history. Safe to re-upload as often as you like: '
      + 'this is the one target where a re-import cannot create duplicates.',
  },
  {
    table: 'daily_km',
...
/** Targets where a re-import CANNOT create duplicates (snapshot-replace semantics). */
export const SAFE_TO_REIMPORT = Object.freeze(
  IMPORT_TARGETS.filter((t) => /REPLACES the whole snapshot/.test(t.notes)).map((t) => t.table),
)
