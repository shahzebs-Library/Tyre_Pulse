...
export default function ApprovalMatrix() {
  ...
  return <div dir={isRTL ? 'rtl' : 'ltr'} className="space-y-5">
    ...
    {coverage && <section className="card space-y-3" aria-label={c.coverage}><h2 className="font-bold">{c.coverage}</h2><p className="text-sm text-[var(--text-muted)]">{c.coverageHint}</p><div className="overflow-x-auto"><table className="w-full text-sm text-start"><thead><tr>{[c.country, c.region, c.vehiclesCol, c.sitesCol, c.routeCol].map(h => <th key={h} className="p-2 text-start">{h}</th>)}</tr></thead><tbody>{coverage.regions.map(r => <tr key={`${r.country}:${r.region}`} className="border-t border-[var(--hairline)] align-top"><td className="p-2">{r.country}</td><td className="p-2">{r.region || c.noRegionShort}</td><td className="p-2">{r.vehicles}</td><td className="p-2 text-xs text-[var(--text-muted)]">{(r.sites || []).filter(Boolean).join(', ')}</td><td className="p-2">{r.route_status === 'matched' ? r.route : <span className="text-amber-600">{r.route_status === 'ambiguous' ? c.ambiguousShort : c.noPublishedRoute}</span>}</td></tr>)}</tbody></table></div>{coverage.unmapped_sites?.length > 0 && <p role="alert" className="text-sm text-amber-600">{c.unmappedSites}: {coverage.unmapped_sites.map(u => `${u.site} (${u.vehicles})`).join(', ')}. <Link className="underline" to="/sites">{c.openSites}</Link></p>}</section>}
