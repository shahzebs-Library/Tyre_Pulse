...
import {
  normalizeDiagramConfig, positionsFromConfig, DEFAULT_DIAGRAM_CONFIG,
 ...
 *  (axles, dual/single wheels, spares, body style, animated accents) with a
 *  live SVG preview. Active layouts replace the app's built-in tyre diagrams
 *  for that vehicle type; built-ins stay the fallback. Table: V268. */
export default function ConsoleVehicleDesigner() {
  ...
  // Distinct fleet vehicle types feed the type picker (canonical UPPER, V245).
  useEffect(() => {
    let alive = true
    ;(async () => {
      try {
  ...
  async function handleSave() {
    ...
    setSaved(false)
    try {
      const saved = await upsertVehicleDiagramConfig({
        vehicle_type: vt,
        label: draft.label,
        config: draft.config,
        active: draft.active,
      ...
      await logAction('update_config', null, 'vehicle_diagram', { vehicle_type: vt, active: saved.active })
      setDraft((d) => ({ ...d, id: saved.id, vehicle_type: saved.vehicle_type }))
      setSaved(true)
      await load()
    } catch (e) {
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Design custom vehicle diagrams per vehicle type: axles, dual or single wheels, spares, body style and
            animated accents. Active designs replace the built-in tyre diagrams across the app for that type.
          </p>
        </div>
        ...
        <div className="xl:col-span-2 rounded-xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  {draft.config.axles.map((axle, i) => (
                    <div key={i} className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-950/60 px-2.5 py-1.5">
              ...
              {/* Accents */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300">Animated accents</label>
                ...
                <p className="text-[11px] text-slate-500">
                  Hazard indicators blink and the beacon pulses in the live diagram. A mixer body also gets a rotating drum.
                  Animations switch off automatically for users who prefer reduced motion.
                </p>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 flex items-center justify-center overflow-auto"
                ...
                <CustomDiagramPreview layout={layout} width={250} />
              </div>
              <p className="text-[11px] text-slate-500">
