...
export function byCountry(rows) {
  ...
  for (const e of m.values()) e.years.sort((a, b) => a.year.localeCompare(b.year))
 ...
 */
export function forecast(years, ahead = 2) {
  const hist = years || []
  if (hist.length < 2) return []
  const lastYear = Number(hist[hist.length - 1].year)
  const fits = {}
  for (const k of [...CATEGORIES, 'total']) fits[k] = linearFit(hist.map((y) => y[k]))
  const out = []
  for (let a = 1; a <= ahead; a++) {
    const idx = hist.length - 1 + a
    const row = { year: String(lastYear + a), forecast: true }
...
export function insights(years) {
  const out = []
  ...
  if (g != null) {
    out.push({
      ...
      text: `Total spend ${g >= 0 ? 'grew' : 'fell'} ${Math.abs(Math.round(g))}% per year on average from ${first.year} to ${last.year}.`,
    })
  }
  ...
  const f = forecast(years, 1)[0]
  if (f && f.total != null) {
    out.push({ tone: 'accent', text: `On the current trend, ${f.year} total spend is projected near ${f.total.toLocaleString()}.` })
  }
  return out
}

/** Everything a country panel needs, in one call. */
export function buildCountryTrend(entry) {
  const years = entry?.years || []
  return {
    country: entry?.country, currency: entry?.currency || '',
    years,
    yoy: yoyTable(years),
    share: latestShare(years),
    cagr: cagr(years),
    forecast: forecast(years, 2),
    insights: insights(years),
  }
}
