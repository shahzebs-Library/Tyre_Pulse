...
/** Existing values of a column (paged) as a lowercase Set, for merge dedup. */
async function existingKeys(table, column) {
  const keys = new Set()
  let from = 0
  const size = 1000
  for (let guard = 0; guard < 500; guard += 1) {
    const { data, error } = await supabase.from(table).select(column).not(column, 'is', null).range(from, from + size - 1)
...
/** Tyre lifecycle rows -> tyre_records. Skips serials already stored (merge). */
export async function insertTyreRecords(rows = [], { onProgress } = {}) {
  const seen = await existingKeys('tyre_records', 'serial_no').catch(() => new Set())
...
export async function insertWorkOrders(rows = [], { onProgress } = {}) {
...
/** Open-job-card snapshot -> open_work_orders. REPLACES the whole list. */
export async function replaceOpenWorkOrders(rows = [], { onProgress } = {}) {
  const { error } = await supabase.from('open_work_orders').delete().not('id', 'is', null)
  if (error) throw error
...
export async function insertVehicleFleet(rows = [], { onProgress } = {}) {
  const seen = await existingKeys('vehicle_fleet', 'asset_no').catch(() => new Set())
