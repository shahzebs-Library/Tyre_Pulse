...
export default function TyreExchange() {
  ...
  function transferTypeBadge(type) {
    ...
    return (
      <span className={`px-2 py-0.5 rounded-full text-xs border ${map[type] || 'bg-gray-800 text-gray-400 border-gray-700'}`}>
        {type}
      </span>
    )
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            Tyre Exchange &amp; Transfer Management
          </h1>
          <p className="text-sm text-gray-400 mt-1">
            Inter-vehicle transfers · Site-to-site movements · Retread custody · Return tracking
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={exportTransfersPdf}
            className="flex items-center gap-1.5 px-3 py-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-200 rounded-lg text-sm transition-colors"
          >
                  ...
                  {showFilters && (
                    ...
                    >
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2">
                        <div>
                          <label className="text-xs text-gray-400 mb-1 block">From Site</label>
                          <select
                        ...
                        <div>
                          <label className="text-xs text-gray-400 mb-1 block">To Site</label>
                          <select
                            value={filterToSite}
                        ...
                        <div>
                          <label className="text-xs text-gray-400 mb-1 block">Transfer Type</label>
                          <select
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {txPagedData.length === 0 ? (
                        ...
                        txPagedData.map(t => (
                          <tr key={t.id} className="border-t border-gray-800 hover:bg-gray-800/30 transition-colors">
                            <td className="px-4 py-3 font-mono text-blue-400 text-xs">{t.serial}</td>
                            <td className="px-4 py-3 text-gray-200">{t.brand}</td>
                            <td className="px-4 py-3 text-gray-400">{t.size}</td>
                            <td className="px-4 py-3 text-gray-300">{t.fromAsset}</td>
                            <td className="px-4 py-3 text-gray-300">{t.toAsset}</td>
                            <td className="px-4 py-3 text-gray-400">{t.fromSite}</td>
                ...
                {txTotalPages > 1 && (
                  <div className="flex items-center justify-between px-4 py-3 border-t border-gray-800">
                    <button
                      ...
                      disabled={txPage === 1}
                      className="flex items-center gap-1 px-3 py-1.5 text-sm text-gray-400 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {retreads.length === 0 ? (
                        ...
                        retreads.map((r, idx) => (
                          <tr key={`${r.serial}-${idx}`} className={`border-t border-gray-800 hover:bg-gray-800/30 transition-colors ${r.overdue ? 'bg-red-900/10' : ''}`}>
                            <td className="px-4 py-3 font-mono text-blue-400 text-xs">{r.serial}</td>
                            <td className="px-4 py-3 text-gray-200">{r.brand}</td>
                            <td className="px-4 py-3 text-gray-400">{r.size}</td>
                            <td className="px-4 py-3 text-gray-300">{r.sentFromAsset}</td>
          ...
          {activeTab === 'custody' && (
            <div className="space-y-6">
                ...
                <div className="flex gap-2 max-w-lg">
                  ...
                  {custodySerial && (
                    <button
                      ...
                      className="px-3 py-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-400 rounded-lg text-sm transition-colors"
                    >
                      <X size={15} />
                    </button>
                  )}
              ...
              {custodySearched && !custodySerial && (
                <div className="text-center py-12 text-gray-500">Enter a serial number to view custody chain.</div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex flex-wrap gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-200">{custodyChain.length}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Unique Vehicles</p>
                      <p className="text-sm text-gray-200">
                        {new Set(custodyChain.map(r => r.asset_no).filter(Boolean)).size}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Unique Sites</p>
                      <p className="text-sm text-gray-200">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-200">{fmtDate(custodyChain[0]?.issue_date)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Last Record</p>
                      <p className="text-sm text-gray-200">{fmtDate(custodyChain[custodyChain.length - 1]?.issue_date)}</p>
                    </div>
                  </div>
                        ...
                        return (
                          <div key={r.id || idx} className="flex items-start">
                            <div className="flex flex-col items-center">
                              <div className={`w-4 h-4 rounded-full ${dotColor} ring-2 ring-gray-900 z-10 mt-6`} />
                              {!isLast && <div className="h-0.5 w-24 bg-gray-700 mt-1.5" style={{ transform: 'translateX(50%)' }} />}
                            </div>
                            <div className="ml-[-8px] mt-10 mr-8 w-40">
                              <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 text-xs space-y-1">
                                ...
                                {r.km_at_removal != null && (
                                  <p className="text-gray-400">End: {r.km_at_removal.toLocaleString()} km</p>
                                )}
                                {r.tread_depth != null && (
                                  <p className="text-gray-400">Tread: {r.tread_depth}mm</p>
                                )}
                                {r.category && (
              ...
              {!custodySearched && (
                <div className="text-center py-16 text-gray-500">
                  ...
                  <p className="text-xs mt-1 text-gray-600">Every fitment, transfer, and removal event will be shown in chronological order.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'pending' && (
            <div className="space-y-4">
                    ...
                    <tbody>
                      {pendingReturns.length === 0 ? (
                        ...
                        pendingReturns.map((p, idx) => (
                          <tr key={`${p.serial}-${idx}`} className={`border-t border-gray-800 hover:bg-gray-800/30 transition-colors ${
                            ...
                            <td className="px-4 py-3 text-gray-200">{p.brand}</td>
                            <td className="px-4 py-3 text-gray-400">{p.size}</td>
                            <td className="px-4 py-3 text-gray-300">{p.removedFrom}</td>
                            <td className="px-4 py-3 text-gray-400">{p.site}</td>
                            <td className="px-4 py-3 text-gray-400 whitespace-nowrap">{fmtDate(p.removalDate)}</td>
                            <td className="px-4 py-3">
                              <span className="px-2 py-0.5 bg-gray-800 text-gray-300 border border-gray-700 rounded-full text-xs">
                                {p.category}
                              </span>
                            </td>
              ...
              {(returnedSerials.length > 0 || writtenOffSerials.length > 0) && (
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                  <p className="text-sm text-gray-400 mb-2">Session Actions</p>
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                  <h3 className="text-sm font-medium text-gray-300 mb-4">Transfers per Month (Last 12 Months)</h3>
                  {transfers.length === 0 ? (
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                  <h3 className="text-sm font-medium text-gray-300 mb-4">Transfer Types Breakdown</h3>
                  {transfers.length === 0 ? (
                    <div className="flex items-center justify-center h-48 text-gray-500 text-sm">
                      No transfer data available
                    </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                <h3 className="text-sm font-medium text-gray-300 mb-4">Most Transferred Serials</h3>
                {(() => {
                  const counts = {}
                  for (const t of transfers) {
                      ...
                      {top.map(([serial, count]) => {
                        ...
                        return (
                          <div key={serial} className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-gray-300 w-12 text-right">{count} transfers</span>
                          </div>
                        )
                      })}
                    </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                <h3 className="text-sm font-medium text-gray-300 mb-4">Transfers by Brand</h3>
                {(() => {
                  const counts = {}
                  for (const t of transfers) {
                  ...
                  if (sorted.length === 0) return <p className="text-sm text-gray-500">No data.</p>
                  const max = sorted[0]?.[1] || 1
                  return (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {sorted.map(([brand, count]) => (
                        <div key={brand} className="flex items-center gap-3">
                          <span className="text-xs text-gray-300 w-28 truncate">{brand}</span>
                          <div className="flex-1 bg-gray-800 rounded-full h-2">
                            <div
                ...
                {siteFlowMatrix.sites.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="text-xs border-collapse">
                      <thead>
                        <tr>
                          <th className="text-left px-3 py-2 text-gray-400 font-medium bg-gray-800 border border-gray-700 min-w-[120px]">
                            From ↓ / To →
                          </th>
                          {siteFlowMatrix.sites.map(site => (
                            <th key={site} className="px-3 py-2 text-gray-400 font-medium bg-gray-800 border border-gray-700 min-w-[80px] text-center">
                              {site}
                            </th>
                          ))}
                          <th className="px-3 py-2 text-gray-400 font-medium bg-gray-800/80 border border-gray-700 text-center">
                            Total Out
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {siteFlowMatrix.sites.map(fromSite => {
                          ...
                          return (
                            <tr key={fromSite}>
                              <td className="px-3 py-2 text-gray-300 font-medium bg-gray-800/40 border border-gray-700">
                                {fromSite}
                              </td>
                              {siteFlowMatrix.sites.map(toSite => {
                                ...
                                return (
                                  <td
                                    key={toSite}
                                    className={`px-3 py-2 text-center border border-gray-700 font-medium ${
                        ...
                        <tr className="bg-gray-800/30">
                          <td className="px-3 py-2 text-gray-400 font-medium border border-gray-700">Total In</td>
                          {siteFlowMatrix.sites.map(toSite => {
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {siteFlowMatrix.sites.map(site => {
                          ...
                          const roleColor = role === 'Net Receiver' ? 'text-green-400' : role === 'Net Sender' ? 'text-orange-400' : 'text-gray-400'
                          return (
                            <tr key={site} className="border-t border-gray-800 hover:bg-gray-800/20">
                              <td className="py-2.5 pr-4 text-gray-200 font-medium">{site}</td>
                              <td className="py-2.5 px-4 text-right text-orange-400">{out}</td>
                              <td className="py-2.5 px-4 text-right text-green-400">{inp}</td>
                              <td className={`py-2.5 px-4 text-right font-semibold ${net > 0 ? 'text-green-400' : net < 0 ? 'text-red-400' : 'text-gray-400'}`}>
                                {net > 0 ? `+${net}` : net}
                              </td>
