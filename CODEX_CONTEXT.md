...
- Enterprise checklist evidence foundation (2026-09-21, branch `feature/checklist-enterprise-governance`): immutable published template revisions, exact snapshots on new submissions, explicit legacy/missing evidence labels and assignment-based 30-day compliance monitoring are implemented. Web detail/PDF reading prefers stored snapshots. Plan and measured production baseline: `audit/enterprise-checklist-governance-plan.md`. Production schedules, approval policies and historical backfills remain intentionally unconfigured pending operational inputs.
