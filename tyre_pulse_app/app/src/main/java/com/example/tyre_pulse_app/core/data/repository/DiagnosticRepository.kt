...
class DiagnosticRepository @Inject constructor(
    ...
    fun getDiagnostics(): Flow<AppDiagnostics> {
        ...
        val pendingFlow: Flow<List<SyncOperationEntity>> = syncDao.getPendingOperations()
        val failedFlow: Flow<Int> = syncDao.getFailedCount()

        return combine(pendingFlow, failedFlow) { pending: List<SyncOperationEntity>, failedCount: Int ->
            AppDiagnostics(
                appVersion = version,
                buildNumber = code,
