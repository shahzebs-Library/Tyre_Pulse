...
class AssetRepository @Inject constructor(
    ...
    suspend fun getAsset(id: String, tenantId: String): Asset {
        val local = assetDao.getAssetById(id, tenantId)
        if (local != null) {
            return json.decodeFromString<Asset>(local.rawData)
        }
        val remote = assetApi.getAsset(id).first()
        assetDao.insertAssets(listOf(remote.toEntity(tenantId)))
        return remote
    }
