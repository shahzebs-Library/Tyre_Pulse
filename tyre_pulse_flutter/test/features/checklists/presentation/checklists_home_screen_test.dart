...
Future<ProviderContainer> _pump(
  ...
  VehicleFleetListOutcome? fleetOutcome,
}) async {
...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(find.text('Master data verified'), findsNothing);
  ...
  testWidgets('typing a fleet-only asset selects it without an assignment',
      (tester) async {
    await _pump(
      tester,
      fleetOutcome: const VehicleFleetListLoaded(
        assets: <VehicleAsset>[
          _pumpVehicle,
          VehicleAsset(id: 'other', assetNo: 'TR-222', vehicleType: 'Truck'),
        ],
        truncated: false,
      ),
    );
    await tester.enterText(find.byType(TextField), 'TR-222');
    await tester.pumpAndSettle();
    expect(find.textContaining('TR-222'), findsWidgets);
    expect(find.byKey(ChecklistsHomeScreenKeys.requiredForAsset), findsNothing);
    expect(tester.takeException(), isNull);
  });

  testWidgets('fleet failure stays visible without claiming synchronization',
      (tester) async {
    await _pump(
      tester,
      fleetOutcome: const VehicleFleetListFailed(AppError.network()),
    );
