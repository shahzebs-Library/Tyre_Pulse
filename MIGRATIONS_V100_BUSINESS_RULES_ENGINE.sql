-- ============================================================================
-- MIGRATIONS_V100_BUSINESS_RULES_ENGINE.sql
-- Phase 14 (roadmap): Business Rules Engine.
--
--  * business_rules — org-configurable "if condition then action" rules.
--    trigger_type 'event' rules evaluate their conditions against V98 domain
...
--
-- Depends on: V98 (domain_events, event_consumers), V99 (notify_role_in_org),
...
COMMENT ON TABLE public.business_rules IS
  'Org-configurable event rules: conditions (ANDed field/operator/value vs event payload) → actions (notify_role, emit_event ''rule.*''). Evaluated by consume_event_rules via V98.';
