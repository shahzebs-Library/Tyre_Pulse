 ...
 *    blocks NOTHING. resolveVehicleType falls back to a 4 wheel Pickup for an
 ...
 *
 * MIRROR: src/lib/tyreCompleteness.js. Change both together. The behavioural
...
import { diagramPositions, resolveVehicleType, isTyrelessEquipment, legacyPositionCode } from './tyreDiagramLayouts'

export type TyreSlotState = 'missing' | 'blank' | 'incomplete' | 'complete'

export interface TyreSlotStatus {
  slot: string
  code: string
  state: TyreSlotState
  reason: string | null
...
export interface TyreCompletenessResult {
  applicable: boolean
  known: boolean
  matched: boolean
  ...
  recorded: number
  complete: number
  slots: TyreSlotStatus[]
  pending: TyreSlotStatus[]
  missing: TyreSlotStatus[]
  blank: TyreSlotStatus[]
  incomplete: TyreSlotStatus[]
  blocked: TyreSlotStatus[]
  extra: string[]
...
export interface TyreCompletenessOptions {
...
function pick(entry: Record<string, unknown> | null, names: string[]): unknown {
...
export interface TyreEntryPair { key: string; entry: Record<string, unknown> }
 ...
 */
export function readTyreEntries(source: unknown): TyreEntryPair[] {
  let tc: unknown = source && typeof source === 'object' && !Array.isArray(source) && 'tyre_conditions' in (source as object)
    ? (source as Record<string, unknown>).tyre_conditions
    : source
  if (typeof tc === 'string') {
  ...
  const pairs: Array<[string, unknown]> = Array.isArray(tc)
    ? tc.map((d, i) => {
      const rec = d as Record<string, unknown> | null
      const named = rec && typeof rec === 'object' && hasText(rec.position)
      return [named ? String((rec as Record<string, unknown>).position) : String(i), d] as [string, unknown]
    })
    : Object.entries(tc as Record<string, unknown>)

  const out: TyreEntryPair[] = []
 ...
 */
export function classifyEntry(entry: Record<string, unknown> | null): EntryClassification {
 ...
 *
 * resolveVehicleType NEVER returns null - an unrecognised type silently becomes
 * a 4 wheel Pickup. That fallback is right for drawing something, and completely
 * wrong for telling somebody a wheel is missing, so it is detected here.
 */
export function layoutIsKnown(vehicleType?: string | null, assetNo?: string | null): boolean {
...
/**
 * @param vehicleType   the machine's type
 * @param assetNo       used only when the type identifies nothing
 * @param tyreConditions the row, or its tyre_conditions, any shape
 */
export function tyreCompleteness(
  vehicleType?: string | null,
  assetNo?: string | null,
  tyreConditions?: unknown,
  opts: TyreCompletenessOptions = {},
): TyreCompletenessResult {
  ...
  const layoutKey = resolveVehicleType(vehicleType, assetNo)
  const slotIds = known ? diagramPositions(String(vehicleType ?? ''), assetNo) : []
  ...
  const bySlot: Record<string, Record<string, unknown>> = {}
  const extra: string[] = []
  ...
  const slots: TyreSlotStatus[] = slotIds.map(slot => {
    const entry = bySlot[slot]
    const c = entry ? classifyEntry(entry) : { state: 'missing' as TyreSlotState, pressure: null, condition: null }
    return {
      slot,
      code: codeOf[slot] || slot,
      state: c.state,
      reason: c.state === 'complete' ? null : (PENDING_REASONS as Record<string, string>)[c.state],
...
function result(input: ResultInput): TyreCompletenessResult {
  const { applicable, known, matched, layoutKey, expected, slots, extra, blocking, summary } = input
  const byState: Record<string, TyreSlotStatus[]> = {}
...
export function pendingCodes(res: TyreCompletenessResult | null | undefined): string[] {
 ...
 */
export function slotStateMap(res: TyreCompletenessResult | null | undefined): Record<string, TyreSlotState> {
  const out: Record<string, TyreSlotState> = {}
...
export function isPendingPosition(res: TyreCompletenessResult | null | undefined, position: string): boolean {
